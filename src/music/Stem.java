...
public class Stem extends Duration implements Comparable<Stem>{
    ...
    public Stem(Staff staff, boolean up) {
        ...
        this.staff = staff;
        isUp = up;
        // staff.sys.stems.add(this); // this is done in the Time class

        addReaction(new Reaction("E-E") { // incrementing flags
        ...
        addReaction(new Reaction("W-W") { // decrementing flags
            public int bid(Gesture g) {
                return bidLineCrossStem(g.vs.yM(), g.vs.xL(), g.vs.xH(), Stem.this);
            }
    ...
    public int yBeamEnd() {
        Head h = lastHead();
        int line = h.line;
        line += (isUp ? -7 : 7); // default extension 1 octave
