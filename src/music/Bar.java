...
public class Bar extends Mass {
    ...
    private static final int FAT = 2, RIGHT = 8, LEFT = 4; // Bits/Flags in bar type
    public Sys sys;
    public int x, barType = 0;
    /*
    barType 0 -> Normal, single thin line
            1 -> Double thin line (Key changes)
            2 -> Fine line
            4-7 -> Fat + thin + dot (End repeat bar)
            8-11 -> Dot + thin + fat (Start repeat bar)
            12 -> Dot + thin + fat + thin + dot (Double repeat bar)
     */
    ...
    public Bar(Sys sys, int x) {
        ...
        addReaction(new Reaction("S-S") { // Cycle through types of bar
